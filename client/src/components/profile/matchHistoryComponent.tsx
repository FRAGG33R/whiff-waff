...
export default function MatchComponents() {
  ...
  const fetchMatchHistoryPage = async () => {
	...
	try {
		...
		if (!token)
			return ;
		const res = await api.get(`/users/historyGame/${(user as userType).userName}?page=${activePage}`, {
			headers: {
          ...
          {matchHistory.map((item : matchHistoryType, index) => {
            return (
              <MatchComponent
                ...
                firstUserName={item.scoreLeft < item.scoreRight ? item.game.playerTwo.userName  : item.game.playerOne.userName}
                secondUserName={item.scoreLeft < item.scoreRight ? item.game.playerOne.userName : item.game.playerTwo.userName}
				firstUserAvatar={item.scoreLeft < item.scoreRight ? item.game.playerTwo.avatar  : item.game.playerOne.avatar}
				secondUserAvatar={item.scoreLeft < item.scoreRight ? item.game.playerOne.avatar : item.game.playerTwo.avatar}
				firstScore={item.scoreLeft > item.scoreRight ? item.scoreLeft : item.scoreRight}
				secondScore={item.scoreLeft < item.scoreRight ? item.scoreLeft : item.scoreRight}
              />)})}
        </div>
        <div className="w-full h-full py-2 flex items-center justify-center ">
          <Pagination max={60} active={activePage} setActive={setActivePage} />
        </div>
      </div>
    </div>
  );
